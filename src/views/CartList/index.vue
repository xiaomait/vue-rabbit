...
// 下单结算
function confirm() {
  const selected = cartStore.cartList.some(item => item.selected === true)
  if (selected) {
    // console.log(selected)
    router.push('/checkout')
  }
  else {
    ElMessage.error('请选择一个商品进行下单')
  }
}
onMounted(() => {
        ...
        <table>
          ...
          <tbody>
            <tr v-for="i in cartStore.cartList" :key="i.id">
              ...
              <td class="tc">
                <el-input-number v-model="i.count" :min="1" :max="9999" @change="cartStore.cartCount(i.skuId, i.count)" />
              </td>
              <td class="tc">
                <p class="f16 red">
